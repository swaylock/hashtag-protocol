...
function shouldBehaveLikeERC721Pausable (errorPrefix, owner, publisher, operator, approved, anotherApproved, other, newOwner, creator) {
    ...
    it('reverts when trying to safeTransferFrom with data', async function () {
      await expectRevert(
        this.token.methods['safeTransferFrom(address,address,uint256,bytes)'](
          owner, newOwner, firstTokenId, mockData, { from: owner },
        ), 'ERC721Pausable: token transfer while paused',
      );
    });
